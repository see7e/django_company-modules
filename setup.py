...
def create_virtual_environment():
    ...
    with tqdm(total=4, desc="Virtual Environment", bar_format="{l_bar}{bar}", ncols=60) as pbar:
        try:
            # Create a virtual environment
            subprocess.run(["python", "-m", "venv", ".venv"], check=True)
        except subprocess.CalledProcessError as e:
            print(color.GRAY + f"Error: Failed to create virtual environment: {e}" + color.ENDC)
            return
...
def create_django_project():
    ...
    project_name = input(color.GRAY + "Enter the name of the Django project: " + color.ENDC)

    # Ask about the type of database to use
    database_type = input(color.GRAY + "Enter the type of database to use (postgresql/mysql/sqlite3): " + color.ENDC)

    # Ask about the Docker image to use
    docker_image = input(color.GRAY + "Enter the Docker image to use (python/ubuntu): " + color.ENDC)
    ...
    with tqdm(total=1, desc="Django Project Creation", bar_format="{l_bar}{bar}", ncols=60) as pbar:
    ...
    time.sleep(0.5)
    # Create a Dockerfile
    with tqdm(total=1, desc="Dockerfile Creation", bar_format="{l_bar}{bar}", ncols=60) as pbar:
    ...
    with tqdm(total=1, desc="docker-compose.yml Creation", bar_format="{l_bar}{bar}", ncols=60) as pbar:
        database_service = ""
    ...
    print(color.GRAY + f"Successfully created Django project '{project_name}' with Docker support and {database_type} database." + color.ENDC)

if __name__ == "__main__":
    # ASCII art animation for progress visualization
    print("Creating Django project:")
    for frame in tqdm(["███▒▒▒▒▒▒▒", "███▒▒▒▒▒▒▒", "███▒▒▒▒▒▒▒", "███▒▒▒▒▒▒▒", "███▒▒▒▒▒▒▒", "███▒▒▒▒▒▒▒", "███▒▒▒▒▒▒▒", "███▒▒▒▒▒▒▒"], desc="Building:", bar_format="{l_bar}{bar}", ncols=60):
        time.sleep(0.01)
